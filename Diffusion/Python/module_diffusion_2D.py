    ...
    for n in range(Nside):
        ...
        if(flag == -5):
            ...
            for i in range(Q+1):
                ...
                if(mixed == True and (ip == ip1 or ip == ip2)): # apply in case of mixed boundary conditions with Dirichlet
...
def diffusion_solver(N,Q,Ne, Np, ax, bx,ay,by, Nelx, Nely, Nx, Ny, Nbound,Nside,icase,Tfinal,c,cfl,\
    ...
    # Initialize values
    q0 = qe
    # computes the second time step solution (backward method)
    # Second time step 
    time = time + dt
    qe,gradq = exact_solution(coord,Np,icase,time)

    AA = Mmatrix - dt*c*Lmatrix
    
    AAinv = linalg.inv(AA)
    qn = Mmatrix@q0
    
    qn =  apply_Dirichlet_BC_vec1(qn,qe,intma,jac_side,imapl,psideh,Np,N,Q,Ne,Nelx,Nside,mixed)
    
#     q = AAinv@qn
    
    # Apply boundary conditions

    qn = apply_Neumann_BC(AAinv,qn,nx,ny,intma,jac_side,imapl,psideh,gradq,Np,N,Q,Nside,c*dt)
    
    q = AAinv@qn

    # BDF2 stuff
    A = 3*Mmatrix - 2*dt*Dhmatrix

    A = apply_Dirichlet_BC_matrix(A,intma,imapl,psideh,Np,N,Q,Nside)
...
#         qp = Rmatrix@qn
        ...
        qn = apply_Neumann_BC(A_inv,qn,nx,ny,intma,jac_side,imapl,psideh,gradq,Np,N,Q,Nside,2*c*dt)
    ...
    #bc = extract_bc(qp,intma,imapl,psideh,coord,N,Q,Nside)
    
    return qe, q, coord, intma, ntime,tf


def diffusion_solver_gmres(N,Q,Ne, Np, ax, bx,ay,by, Nelx, Nely, Nx, Ny, Nbound,Nside,icase,Tfinal,c,cfl,\
                     kstages,time_method,alpha1,beta1,x_boundary,y_boundary, mixed,mxit,stol):
    ...
    (q,flag) = gmres(A1,qn, maxiter=mxit, tol=stol, callback = counterg)
    
    # Apply boundary conditions

    q = apply_Neumann_BC(A1,q,nx,ny,intma,jac_side,imapl,psideh,gradq,Np,N,Q,Nside,c*dt,mxit,stol)
    ...
    for itime in range(1,ntime+1):
        ...
        (qp,flag) = gmres(Rmatrix,qn, maxiter=mxit, tol=stol, callback = counterg)
        #print('iterations = ',counterg.i)

        # Apply boundary conditions

        qp = apply_Neumann_BC(A,qp,nx,ny,intma,jac_side,imapl,psideh,gradq,Np,N,Q,Nside,2*c*dt,mxit,stol)
