...
def Element_matrix_inv_DG(coord,Ne,N,Q, wght,l_basis,integration_type):
    ...
    if(integration_type == 1):
        ...
        Me = zeros((Ne,N+1))       # initialisation of the matrix
        Me_inv = zeros((Ne,N+1)) 
        for e in range(1,Ne+1):
            x = coord[:,e-1]
            dx=x[-1]-x[0]
...
def dss(rhs,intma,Mmatrix_h,Mmatrix_u,Np,Ne,N, iperiodic):
    

    #Initialize
    rhs_continuous = zeros((Np,2))
    ...
    for e in range(1, Ne+1):
        for i in range(N+1):
            ...
            for k in range(2):
                rhs_continuous[ip,k] = rhs_continuous[ip,k] + rhs[e-1, k, i]
                           
    #Scatter Solution to make it Local
    rhs = zeros((Ne,2, N+1))
    rhs_continuous[:,0] = Mmatrix_h@rhs_continuous[:,0]
    
    rhs_continuous[:,1] = Mmatrix_u@rhs_continuous[:,1]
    rhs_continuous[0,1] = 0
    rhs_continuous[-1,1] = 0
    ...
    for e in range(1,Ne+1):
        for i in range(N+1):
            ...
            ip = int(iperiodic[ip])
            for k in range(2):
                rhs[e-1,k,i] = rhs_continuous[ip,k]
            
    return rhs
      
# Exact solution
def exact_solution(coord,Ne,N,time, case,ax,bx):
    ...
    for e in range(1,Ne+1):
        for i in range(N+1):
            ...
            if(case == 1): #Gaussian wave with flat bathymetry
                ...
                gravity = 1
                qe[e-1,0,i] = 0.5*cos(2*pi*x)*cos(2*pi*time)
                qe[e-1,1,i] = 0.5*sin(2*pi*x)*sin(2*pi*time)
                qb[i,e-1] = 1
                
            elif(case == 5):  
                
                gravity = 1
                qe[e-1,0,i] = 1 + (2/5)*exp(-5*xbar**2)
                qe[e-1,1,i] = 0
                qb[i,e-1] = 0.0
...
def create_rhs(qp,qb,coord,Ne,N,Q,wnq,l_basis,dl_basis,h_eps,gravity,delta_nl):
       ...
       #Store Coordinates
        ...
        for k in range(Q+1):
            ...
            for j in range(N+1):
                ...
                l_k  = l_basis[j,k]
                dldx = dl_basis[j,k]*ksi_x
                hu_k = hu_k + hu_e[j]*l_k
                hb_k = hb_k + hb_e[j]*l_k
                hs_k = hs_k + hs_e[j]*l_k
                dbdx = dbdx + hb_e[j]*dldx

            h_k = hs_k + hb_k

            flux_h = hu_k
            flux_hu = (hu_k**2/h_k + 0.5*gravity*hs_k**2)*delta_nl + gravity*hb_k*hs_k*(1-delta_nl)
            ...
            for i in range(N+1):
                ...
                rhs[e-1,1,i] = rhs[e-1,1,i] + wq*(dldx_i*flux_hu - l_i*gravity*hs_k*dbdx)

    return rhs
                
    
    
def BCs(rhs,qp,qb,gravity,Ne,N,delta_nl):
    ...
    flux_h = hu_k
    flux_hu = (hu_k**2/(h_k) + 0.5*gravity*hs_k**2)*delta_nl + gravity*hb_k*hs_k*(1-delta_nl)
    rhs[Ne-1,0,N] = rhs[Ne-1,0,N] - flux_h
    rhs[Ne-1,1,N] = rhs[Ne-1,1,N] - flux_hu
    ...
    flux_h = hu_k
    flux_hu = (hu_k**2/(h_k) + 0.5*gravity*hs_k**2)*delta_nl + gravity*hb_k*hs_k*(1-delta_nl)
    rhs[0,0,0] = rhs[0,0,0] + flux_h
    rhs[0,1,0] = rhs[0,1,0] + flux_hu
...
def filter_func(N,xn,xmu,filter_type,weight_type):
    ...
    for i in range(N+1):
        ...
        for j in range(N+1):
            jj = j
            L0,L0_1,L0_2 = Legendre_deriv(jj,x)
...
def productInvMass_rhs(M_inv,N,Ne,rhs,integration_type):
    
    # For inexact integration
    if(integration_type == 1):
        ...
        for e in range(1,Ne+1):
            ...
            #    rhs_h[i] = rhs[e-1,0,i]
            #    rhs_hu[i] = rhs[e-1,1,i]
            
            # Multiply by inverse mass matrix
            rhs[e-1,0,:] = mass1*rhs_h
            rhs[e-1,1,:] = mass1*rhs_hu
...
def rusanov_flux(hl,ul,hr,ur,hsl,hsr,hbl,hbr,gravity,diss,delta_nl):
    ...
    # Compute Wave Speeds 
    lamb_l = (abs(ul) + sqrt(gravity*hl))*delta_nl + sqrt(gravity*hbl)*(1-delta_nl)
    lamb_r = (abs(ur) + sqrt(gravity*hr))*delta_nl + sqrt(gravity*hbr)*(1-delta_nl)
    ...
    flux_h = 0.5*(fl + fr - diss*abs(lamb)*(hsr - hsl))
    
    #Momentum Flux
    fl = (Ul**2/(hl) + 0.5*gravity*hsl**2)*delta_nl + gravity*hsl*hbl*(1-delta_nl)
    fr = (Ur**2/(hr) + 0.5*gravity*hsr**2)*delta_nl + gravity*hsr*hbr*(1-delta_nl)

    flux_U = 0.5*(fl + fr - diss*abs(lamb)*(Ur - Ul))
    
    return flux_h, flux_U

def HLL_flux(hl,ul,hr,ur,hsl,hsr,hbl,hbr,gravity,diss,delta_nl):
    ...
    # Compute Wave Speeds 
    lamb_l = min(ul - sqrt(gravity*hl), ur -sqrt(gravity*hr))*delta_nl + sqrt(gravity*hbl)*(1-delta_nl)
    lamb_r = max(ul + sqrt(gravity*hl), ur + sqrt(gravity*hr))*delta_nl + sqrt(gravity*hbr)*(1-delta_nl)
    
    # Mass Flux
    fl_h = Ul
    fr_h = Ur
    f_star = (lamb_r*fl_h - lamb_l*fr_h + lamb_l*lamb_r*(hsr - hsl))/(lamb_r-lamb_l)
    flux_h = 0
    if(lamb_l >=0):
        flux_h = fl_h
    elif(lamb_l <= 0 <= lamb_r):
        flux_h = f_star
    elif(lamb_r <= 0):
        flux_h = fr_h
    ...
    f_star = (lamb_r*fl - lamb_l*fr + lamb_l*lamb_r*(Ur - Ul))/(lamb_r-lamb_l)
    flux_U = 0
    if(lamb_l >=0):
        flux_U = fl
    elif(lamb_l <= 0 <= lamb_r):
...
def roe_flux(hl,ul,hr,ur,hsl,hsr,hbl,hbr,gravity,diss,delta_nl):
    ...
    #Momentum Flux
    fU_l = (Ul**2/(hl) + 0.5*gravity*hsl**2)*delta_nl + gravity*hsl*hbl*(1-delta_nl)
    fU_r = (Ur**2/(hr) + 0.5*gravity*hsr**2)*delta_nl + gravity*hsr*hbr*(1-delta_nl)
...
def flux_dg(rhs,qp,qb,Ne,N,diss,h_eps,gravity,delta_nl,time,flux_type):
    ...
    for e in range(1,Ne+2):
        ...
        # Apply rusanouv flux
        if(flux_type == 'rusa'):
            flux_h, flux_hu = rusanov_flux(hl,ul,hr,ur,hsl,hsr,hbl,hbr,gravity,diss,delta_nl)
        elif(flux_type == 'hll'):
            flux_h, flux_hu = HLL_flux(hl,ul,hr,ur,hsl,hsr,hbl,hbr,gravity,diss,delta_nl)
        elif(flux_type == 'roe'):
            flux_h, flux_hu = roe_flux(hl,ul,hr,ur,hsl,hsr,hbl,hbr,gravity,diss,delta_nl)
...
def Shu_Positivity_Preserving_limiter(qp,Ne,l_basis,N,Q,wq,qb,h_eps):
...
def cg_dgSolver(N,Q,nel, Np, ax, bx, integration_type, u, CFL, Tfinal, kstages, icase, \
                delta_nl,h_eps,plot_movie,xmu,ifilter,diss,method_type,\
    ...
    t_time = 0
    qe,qb,gravity = exact_solution(coord,nel,N,t_time,icase,ax,bx)

    # Initial mass
    #mass0 = massInit(wnq, nel, N, Q, l_basis, qe, qb, coord)
    ...
    if(method_type == 'cg'):
        ...
        Mmatrix_u = DSS_operator(Me, nel, N, Np, intma, coord, iperiodic)
        Mmatrix_h = DSS_operator(Me, nel, N, Np, intma, coord, iperiodic)
        ...
        Mmatrix_u_inv = linalg.inv(Mmatrix_u)
        Mmatrix_h_inv = linalg.inv(Mmatrix_h)
    
    elif(method_type == 'dg'):
        
        mass,mass_inv = Element_matrix_inv_DG(coord,nel,N,Q, wnq,l_basis,integration_type)
    ...
    if(plot_movie == True):
        ...
        plt.subplot(211)
        movie1, = plt.plot(x_sol,qe_p,'b-', label = 'DG',markersize=10)
        movie_e1, = plt.plot(x_sol,qe_p,'r--',label = 'Exact',markersize=10)
        plt.legend(title="Height")
        Title = 'Shallow water: t = {:.4f}'
        htitle = plt.title(Title.format(t_time),fontsize=18)
        plt.ylabel('h', fontsize = 15)

        plt.subplot(212)
        movie2, = plt.plot(x_sol,qe_u,'b-',label = 'DG',markersize=10)
        movie_e2, = plt.plot(x_sol,qe_u,'r--',label = 'Exact',markersize=10)
    ...
    for itime in range(1,ntime+1):
        ...
        for ik in range(1,kstages+1):
            ...
            rhs = create_rhs(qp,qb,coord,nel,N,Q,wnq,l_basis,dl_basis,h_eps,gravity,delta_nl)

            if(method_type == 'cg'):
            
                rhs = BCs(rhs,qp,qb,gravity,nel,N,delta_nl)

                rhs = dss(rhs,intma,Mmatrix_h_inv,Mmatrix_u_inv,Np,nel,N, iperiodic)
            
            elif(method_type == 'dg'):

                rhs = flux_dg(rhs,qp,qb,nel,N,diss,h_eps,gravity,delta_nl,t_time,flux_type)

                rhs = productInvMass_rhs(mass_inv,N,nel,rhs,integration_type)
            ...
            if(limiter == 1):
                ...
                qp = Shu_Positivity_Preserving_limiter(qp,nel,l_basis,N,Q,wnq,qb,h_eps)
        ...
        if(plot_movie == True):
            ...
            movie_e2.set_ydata(qe_u)
            #htitle2.set_text(Title2.format(t_time))
            plt.ylim([-1.0,1])
            plt.pause(0.1)
    ...
    return qe_p,qe_u, q_p,q_u, x_sol, coord, intma
