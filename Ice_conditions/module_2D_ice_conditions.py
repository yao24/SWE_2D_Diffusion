...
def grid_2D(Np, Ne, Nbound, Nex, Ney, N, Q, Xn, ax, bx,ay,by,bound_index):
...
def create_Dmatrix(u,v,intma,jac,ksi_x,ksi_y,eta_x,eta_y,l_basis,dl_basis,Np,Ne,N,Q,q):
            ...
            for k in range(Q+1):
                ...
                for n in range(N+1):
                    for m in range(N+1):
                        jp = int(inode[m,n])
                        #print(l_basis[n,l])
                        q_k = q_k+l_basis[m,k]*l_basis[n,l]*q[jp]
                #print(q_k)
                #Loop through I points
                for j in range(N+1):
                    for i in range(N+1):
                        ...
                        dhdx_i = h_e*e_x + h_n*n_x
                        dhdy_i = h_e*e_y + h_n*n_y
                        rhs[ip,0] += wq*dhdx_i*q_k*u_k
                        rhs[ip,1] += wq*dhdy_i*q_k*v_k
                
    return rhs

def Dmatrix_2D(u,v,intma,jac,ksi_x,ksi_y,eta_x,eta_y,l_basis,dl_basis,Np,Ne,N,Q):
    ...
    for ip in range(Np):
        q = zeros(Np)
        q[ip] = 1
        rhs = create_Dmatrix(u,v,intma,jac,ksi_x,ksi_y,eta_x,eta_y,l_basis,dl_basis,Np,Ne,N,Q,q)
...
#     cn = 200*(3*X-sin(3*X))/(3*X*(3*X-sin(3*X)*cos(3*X)))
    ...
    cn = array([47.0448633,  45.14125027, 21.35860322, 19.34033077, 12.96744052])
    X = array([0.72487534, 1.66788175, 2.67948759, 3.70984781, 4.74736721])
    
    un = 0

    for n in range(len(cn)):

        un += cn[n]*exp(-(X[n]**2)*t/25)*sin(X[n]*x)
...
def exact_solution(coord,Np,icase,time):
    ...
    if(icase == 1):
        ...
        qe = exp(x+y+2*time)
        gradq[:,0] = exp(x+y+2*time)
        gradq[:,1] = exp(x+y+2*time)
        ...
        qe = cos(pi*x)*cos(pi*y)*exp(-2*pi**2*time)
        gradq[:,0] = -pi*sin(pi*x)*cos(pi*y)*exp(-2*pi**2*time)
        gradq[:,1] = -pi*cos(pi*x)*sin(pi*y)*exp(-2*pi**2*time)
        ...
        qe = 1.0/(4.0*time+1.0)*exp(-(x-0.5)**2/(0.01*(4.0*time+1.0)))*exp(-(y-0.5)**2/(0.01*(4.0*time+1.0)))
        gradq[:,0] = (-2*(x-0.5)/(0.01*(4.0*time+1.0)))*qe
        gradq[:,1] = (-2*(y-0.5)/(0.01*(4.0*time+1.0)))*qe
        ...
        qe = sin(pi*x)*exp(-time*pi**2)
        gradq[:,0] = pi*cos(pi*x)*exp(-time*pi**2)
        gradq[:,1] = 0.0
...
def apply_Dirichlet_BC_zero(Rvector,intma,jac_side,imapl,psideh,Np,N,Q,Ne,Nelx,Nside,mixed):
...
def apply_Neumann_BC(Matrix,q,nx,ny,intma,jac_side,imapl,psideh,gradq,Np,N,Q,Nside,dt,mxit=None,stol=None):
    ...
    for n in range(Nside):
        ...
        if(flag == -4):
            ...
            for i in range(Q+1):
                ...
                ndp = 0.0 #nxl*gradq[ip,0] + nyl*gradq[ip,1]

                B[ip] += wq*ndp

    if(bol and (mxit==None or stol==None)):
        
        B1 = Matrix@B
        q = q + dt*B
    else:
        (B1,flag) = gmres(Matrix,B, maxiter=mxit, tol=stol)
        q = q + dt*B
...
def apply_Robin_BC(Mmatrix_inv,q,nx,ny,intma,jac_side,imapl,psideh,Np,Q,Nside,coord, qe,gradq,dt,beta):
    ...
    if(bol):
        B = Mmatrix_inv@(B + beta*B1)

        q += dt*B
        
    return q

def apply_ice_bc_new(Matrix,qT,qS,hT, c1, bcst1,hB, c2, bcst2,intma,jac_side,imapl,psideh,Np,N,Q,Nside,dt,Nelx,\
                     gammaS,gammaT, cw, Li, ci, Ti, b, c, pb, a, Kf, Mf, coefF,Meltrate,SaltB, vType,mxit=None,stol=None):
        ...
        if(flag == -7):
            ...
            for i in range(Q,-1,-1):
                ...
                if(vType == "heat"):
                    ...
                    #print('{:.16e}'.format(ndp))
                    #print(ip)
                if(vType == "salt"):
                    ndp = hB(bcst2,SB, Sw, gammaS,V)
...
def extract_bc(qp,intma,imapl,psideh,coord,N,Q,Nside):
    ...
    for n in range(Nside):
        ...
        if(flag == -7):
            for i in range(Q,-1,-1):
                ...
                bc1.append(qp[ip])
                #print(qp[ip])
                bc2.append(coord[ip,1])
                #print(ip)
                
    #bc = bc1[where(bc1!=0)]
    
    return array([bc2,bc1])

def extract_profile(qp,intma,imapl,coord,N,Nely):
...
def ice_ocean_Solver(N,Q,Ne, Np, ax, bx,ay,by, integration_type, hT, hB, cst1, c1, bcst1,cst2, c2,bcst2,\
                     ...
                     coefF, Meltrate, SaltB,Nelx, Nely, Nx, Ny, Nbound,Nside,\
    ...
    AAT = Mmatrix - dt*c1*Lmatrix
    AAS = Mmatrix - dt*c2*Lmatrix
    
    AATinv = linalg.inv(AAT)
    AASinv = linalg.inv(AAS)
    Tn = Mmatrix@T0
    Sn = Mmatrix@S0
    ...
    T = apply_ice_bc_new(AATinv,T,S,hT, c1, bcst1,hB, c2,\
                              bcst2,intma,jac_side,imapl,psideh,Np,N,Q,Nside,dt,Nelx,\
                     gammaS,gammaT, cw, Li, ci, Ti, b, c, pb, a, Kf, Mf, coefF,Meltrate,SaltB,"heat")
    
    S = apply_ice_bc_new(AASinv,T,S,hT, c1, bcst1,hB, c2,\
                              bcst2,intma,jac_side,imapl,psideh,Np,N,Q,Nside,dt,Nelx,\
                     gammaS,gammaT, cw, Li, ci, Ti, b, c, pb, a, Kf, Mf, coefF,Meltrate,SaltB,"salt")
    
    T = apply_Dirichlet_BC_vec1(T,Te,intma,jac_side,imapl,psideh,Np,N,Q,Ne,Nelx,Nside,mixed)
    S = apply_Dirichlet_BC_vec1(S,Se,intma,jac_side,imapl,psideh,Np,N,Q,Ne,Nelx,Nside,mixed)

    # End of IRK time integration

    AT = 3*Mmatrix - 2*dt*c1*Lmatrix
    AS = 3*Mmatrix - 2*dt*c2*Lmatrix
    ...
    for itime in range(2,ntime+1):
        ...
        Tp = apply_ice_bc_new(AT_inv,Tp,Sp,hT, c1, bcst1,hB, c2,\
                              bcst2,intma,jac_side,imapl,psideh,Np,N,Q,Nside,2*dt,Nelx,\
                     gammaS,gammaT, cw, Li, ci, Ti, b, c, pb, a, Kf, Mf, coefF,Meltrate,SaltB,"heat")
        
        Sp = apply_ice_bc_new(AS_inv,Tp,Sp,hT, c1, bcst1,hB, c2,\
                              bcst2,intma,jac_side,imapl,psideh,Np,N,Q,Nside,2*dt,Nelx,\
                     gammaS,gammaT, cw, Li, ci, Ti, b, c, pb, a, Kf, Mf, coefF,Meltrate,SaltB,"salt")
    ...
    time_f = perf_counter() - t0        # End of the timing
    
    bc_ice = extract_bc(T,intma,imapl,psideh,coord,N,Q,Nside)
    temp_profile = extract_profile(T,intma,imapl,coord,N,Nely)
        
    return S, T, coord,time_f, bc_ice,temp_profile


def ice_ocean_Solver_gmres(N,Q,Ne, Np, ax, bx,ay,by, integration_type, hT, hB, cst1, c1, bcst1,cst2, c2,bcst2,\
                     Tw, gammaS,gammaT, cw, Li, ci, Ti, b, c, pb, a, Sw, Kf, Mf, cfl, Tfinal,\
                     coefF, Meltrate, SaltB,Nelx, Nely, Nx, Ny, Nbound,Nside,\
                     kstages,x_boundary,y_boundary,mixed,mxit,stol):
    '''
    This function is CG/DG solver for 1D advection-diffusion equation
    ...
    AAT = csr_matrix(AAT)
    AAS = csr_matrix(AAS)
    Mmatrix = csr_matrix(Mmatrix)
    
    Tn = Mmatrix@T0
    Sn = Mmatrix@S0
    
    (T,flag) = gmres(AAT,Tn, maxiter=mxit, tol=stol)
    (S,flag) = gmres(AAS,Sn, maxiter=mxit, tol=stol)
    
    # Apply boundary conditions
    T = apply_ice_bc_new(AAT,T,S,hT, c1, bcst1,hB, c2,\
                              bcst2,intma,jac_side,imapl,psideh,Np,N,Q,Nside,dt,Nelx,\
                     gammaS,gammaT, cw, Li, ci, Ti, b, c, pb, a, Kf, Mf, coefF,Meltrate,SaltB,"heat",mxit,stol)
    
    S = apply_ice_bc_new(AAS,T,S,hT, c1, bcst1,hB, c2,\
                              bcst2,intma,jac_side,imapl,psideh,Np,N,Q,Nside,dt,Nelx,\
                     gammaS,gammaT, cw, Li, ci, Ti, b, c, pb, a, Kf, Mf, coefF,Meltrate,SaltB,"salt",mxit,stol)
    ...
    RmatrixT = Mmatrix_inv@AT
    RmatrixS = Mmatrix_inv@AS
    RmatrixT = csr_matrix(RmatrixT)
    RmatrixS = csr_matrix(RmatrixS)
    AT = csr_matrix(AT)
    AS = csr_matrix(AS)
    ...
    for itime in range(2,ntime+1):
        ...
        (Tp,flag) = gmres(RmatrixT,Tn, maxiter=mxit, tol=stol, callback = countergT)
        (Sp,flag) = gmres(RmatrixS,Sn, maxiter=mxit, tol=stol, callback = countergS)

        # Apply boundary conditions
       
        Tp = apply_ice_bc_new(AT,Tp,Sp,hT, c1, bcst1,hB, c2,\
                              bcst2,intma,jac_side,imapl,psideh,Np,N,Q,Nside,2*dt,Nelx,\
                     gammaS,gammaT, cw, Li, ci, Ti, b, c, pb, a, Kf, Mf, coefF,Meltrate,SaltB,"heat",mxit,stol)
        
        Sp = apply_ice_bc_new(AS,Tp,Sp,hT, c1, bcst1,hB, c2,\
                              bcst2,intma,jac_side,imapl,psideh,Np,N,Q,Nside,2*dt,Nelx,\
                     gammaS,gammaT, cw, Li, ci, Ti, b, c, pb, a, Kf, Mf, coefF,Meltrate,SaltB,"salt",mxit,stol)
    ...
    print('\tIterations T = ',countergT.i)
    print('\tIterations S = ',countergS.i)
